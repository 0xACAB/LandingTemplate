...
export default function Pricing({ ref }) {
	const { pricing } = config;
	const { items, title } = pricing;
	const [firstPlan, secondPlan, thirdPlan] = items;

	return (
		<section className={`bg-background py-0`} id="pricing" ref={ref}>
			<div className={`container mx-auto px-2 pt-4 pb-0 text-primary`}>
				<h1
				...
				>
					<div
						className={`flex flex-col w-5/6 lg:w-1/6 mx-auto lg:mx-0 rounded-none lg:rounded-l-lg bg-background mt-4`}
					>
						<div
					...
					<div
						className={`flex flex-col w-5/6 lg:w-1/6 mx-auto lg:mx-0 rounded-none lg:rounded-l-lg bg-primary mt-4`}
					>
						<div
							className={`flex-1 bg-background text-gray-600 rounded-t rounded-b-none overflow-hidden shadow`}
						>
